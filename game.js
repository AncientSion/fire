...
function Game(data, userid){
	...
	this.hasNoFires = function(){
		...
		if (data.length){
			...
			for (var i = 0; i < data.length; i++){
				html += "<p onclick='game.switchToUnit(" + data.id +     "')>" + data[i].name + " #" + data[i].id;
			}
...
/*		var w = $(div).width();
	...
	this.animateUnitMovement = function(){
		...
		if (elapsed > window.fpsInterval){
			...
			for (var i = 0; i < game.ships.length; i++){
				if (game.ships[i].deployed){
					if (this.ships[i].flight && !this.animFlight || this.ships[i].ship && !this.animShip){
						game.ships[i].draw();
						continue;
					}
