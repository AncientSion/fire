...
this.doAnimateMovement = function(){
	...
	this.setAllObstaclesNextMoves = function(){
		console.log("setAllObstaclesNextMoves");
		for (var i = 0; i < this.ships.length; i++){
	...
	this.setAllObstaclesImages = function(){
		console.log("setAllObstaclesImages");
		for (var i = 0; i < this.ships.length; i++){
...
Game.prototype.getCallback = function(){
	console.log("GET " + this.callback[0]);
	if (this.callback[0] == ""){
		console.log("no callback set!"); return;
	}
...
Game.prototype.handleSingleUnitExplos = function(i){
	console.log("handleSingleUnitExplos");
...
Game.prototype.resolveDamageControl = function(){
	...
	if (toDo){
		...
		window.startTime = then;
		this.animating = 1;
		this.animateDeployOut();
	} else this.resolveObstacleMovement();
}

Game.prototype.animateDeployOut = function(){
	anim = window.requestAnimationFrame(game.animateDeployOut.bind(this));
	window.now = Date.now();		
	window.elapsed = window.now - window.then;
...
Game.prototype.handlePostFireMoves = function(){
	...
	if (this.phase == 2){
		//this.setCallback("handleForcedMoves");
		this.handleForcedMoves();
	} else this.handleAllUnitExplos();
	//} else this.setCallback("handleAllUnitExplos");
			...
			.each(function(i){
				...
				$(this).click(function(e){
					...
					if (game.phase == -1){
						...
						else if (!game.deploying && !game.aUnit && game.getUnit($(this).data("unitid")).canDeploy()){
							$(this).addClass("selected");
